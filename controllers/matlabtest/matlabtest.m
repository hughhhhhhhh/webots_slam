...
encoder_value(1) = ENCODER_UNIT * (wb_position_sensor_get_value(left_position_sensor) - left_encoder_offset);
encoder_value(2) = ENCODER_UNIT * (wb_position_sensor_get_value(right_position_sensor) - right_encoder_offset);   
    % /*
     % * simply turn the wheels in the direction corresponding to the
     % * objectif position, when it is reached, stop the wheel.
     % */    
    if  encoder_value(1) ~= goal(1) |  encoder_value(2) ~= goal(2)
         for i=1:2
       ...
       % * positions and we reset the encoders.
          ...
          for i=1:3
                 goal(i) = 20 * (rand() / RAND_MAX) - 10;
                  ...
                  Y = sprintf('epuck Goal position for the encoders : %d %d\n',goal(1), goal(2));
                   disp(Y);
           end
    end
