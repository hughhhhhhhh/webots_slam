...
// float obstacle[OBSTACLE_SIZE][2] = {{0, 0}, {0, 0}, {0, 0}, {0, 0}};
...
// the goal
 float goal_x[4] = {0.36,0,0,0.4};
 float goal_y[4]= {0.245,0,0.4,0.4};
 int goaltest= 0;
  float pos[3] = {0,0,0};
double receiverx;
double receivery;
...
static float compute_force(int goal_num,float pos_x, float pos_y, float *force_x, float *force_y);
...
// static void init_display() {
  ...
  // for (i = 0; i < width; i++) {
    // for (j = 0; j < height; j++) {
      // pot = (int)(70 * compute_force(0,MAP_WIDTH * i / width, MAP_HEIGHT * j / height, &fx, &fy));
...
static float compute_force(int goal_num,float pos_x, float pos_y, float *force_x, float *force_y) {
  ...
  float d_goal = sqrt((goal_x[goal_num] - pos_x) * (goal_x[goal_num] - pos_x) + (goal_y[goal_num] - pos_y) * (goal_y[goal_num] - pos_y));

  // attractive part (go to the goal)
  f_x += k_att * (goal_x[goal_num] - pos_x) / d_goal;
  f_y += k_att * (goal_y[goal_num] - pos_y) / d_goal;
...
static int run(int goal_num) {
  ...
  float d_goal = sqrt(pow(goal_x[goal_num] - x, 2) + pow(goal_y[goal_num] - y, 2));
  if (d_goal > 0.15) {
    compute_force(goal_num,x, y, &fx, &fy);
   // if (VERBOSE > 0)
    //  printf("Current position is (%f,%f,%f),goal position is (%f,%f)\n", x, y,theta,goal_x[goal_num], goal_y[goal_num]);
    //if (VERBOSE > 0)
...
if (goaltest == 0){
  ...
  while (wb_receiver_get_queue_length(receiver) > 0) {
   wb_receiver_set_channel(receiver,9);
       const double *pos = wb_receiver_get_data(receiver);
//printf("received value y z x :%f %f %f %f\n",pos[0],pos[1],pos[2],pos[15]);
   receiverx  = pos[2];
   receivery =  pos[0];
  ot.result.x = receiverx;
  ot.result.y = receivery;
   run(0);
...
//printf("received value y z x :%f %f %f %f\n",pos[0],pos[1],pos[2],pos[15]);
  ...
  run(1);
   // if (rotation1 == 0 && (pos[15]-3.14>0.1 || pos[15]-3.14<-0.1)){
...
//printf("received value y z x :%f %f %f %f\n",pos[0],pos[1],pos[2],pos[15]);
  ...
  ot.result.y = receivery;
run(2);
     // if (rotation2== 0 && (pos[15]-3.14>0.1 || pos[15]-3.14<-0.1)){
    // wb_motor_set_velocity(left_motor, -1);
    // wb_motor_set_velocity(right_motor, 1);
 // }else{
...
//printf("received value y z x :%f %f %f %f\n",pos[0],pos[1],pos[2],pos[15]);
  ...
  ot.result.y = receivery;
run(3);
     // if (rotation3== 0 && (pos[15]-3.14>0.1 || pos[15]-3.14<-0.1)){
    // wb_motor_set_velocity(left_motor, -1);
    // wb_motor_set_velocity(right_motor, 1);
 // }else{
