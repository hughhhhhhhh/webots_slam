...
int main(int argc, char *argv[]) {
  ...
  node = wb_supervisor_field_get_mf_node(children, 5);
  field = wb_supervisor_node_get_field(node, "translationStep");
     wb_supervisor_field_set_sf_float(field,0.02);

  gravity = wb_supervisor_field_get_sf_float(field);
  printf("WorldInfo.gravity = %f\n", gravity);
